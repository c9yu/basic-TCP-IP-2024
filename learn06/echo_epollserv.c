...
#define BUF_SIZE 100      // 버퍼의 크기를 정의
#define EPOLL_SIZE 50     // epoll 이벤트의 최대 수를 정의
	...
	while(1)
	{
		...
		for(i=0; i<event_cnt; i++)
		{
			if(ep_events[i].data.fd==serv_sock)  // 서버 소켓에 이벤트 발생 시
			{
				...
				clnt_sock=
					accept(serv_sock, (struct sockaddr*)&clnt_adr, &adr_sz);  // 클라이언트 연결 수락
				event.events=EPOLLIN;
			...
			else
			{
				...
				{
					epoll_ctl(
						epfd, EPOLL_CTL_DEL, ep_events[i].data.fd, NULL);  // epoll 인스턴스에서 소켓 삭제
...
{
	fputs(buf, stderr);  // 에러 메시지를 표준 에러 출력에 출력
	fputc('\n', stderr);
	exit(1);  // 프로그램 종료
}
